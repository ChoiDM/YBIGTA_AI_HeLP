...
def process_patient(i, patient, file_list, norm, do_resample, features, target_voxel):
    ...
    try:
        ...
        elif norm == 'lesion_based':
            FLAIR_array = normalization(FLAIR_array, INFARCT_array, size = 8)
        else:
            raise ValueError("Value of 'norm' parameter should be 'lesion_based' of 'ws'")
...
#---------------------------------------------------------------------------------------------------------------#
def train_data_loader(pos_dir='/data/train/positive/', neg_dir='/data/train/negative/', norm='lesion_based',
                      do_resample=True, do_shuffle=True, do_minmax=True,
                      features = ['firstorder', 'shape'], target_voxel = (0.65, 0.65, 3), path="/data"):
    # File List
    pos_file_list = glob(os.path.join(pos_dir, "*"))
    neg_file_list = glob(os.path.join(neg_dir, "*"))
...
#----------------------------------------------------------------------------------------------------------------#
def test_data_loader(test_dir='/data/test/', norm='lesion_based',
                     do_resample=True, do_minmax=True,
                     features = ['firstorder', 'shape'], target_voxel = (0.65, 0.65, 3), path="/data"):
    
    # File List
    test_file_list = glob(os.path.join(test_dir, "*"))
